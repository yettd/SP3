...
public:
	...
	bool bIsActive;
	void SetEL(std::vector<CEnemy2D*> EL);
	
	void SetPotrolSpot(glm::vec2 spot);

	void SetID(int i);

	void AddEnemy(CEnemy2D* e);
	float hungry = 100;
...
protected:
	...
	void GetTarget(std::vector<CEnemy2D*> enemySet);
	//myStuff;
	bool CheckPositionOfObject(glm::vec2 something);

	float hp=11;
	float speed=1;
	std::vector<CEnemy2D*> enemylist;
	CEnemy2D* target = NULL;
