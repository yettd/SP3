...
bool PickUP::Init(void)
{
	...
	{
		if (cMap2D->FindValue(id[i], uiRow, uiCol) == true)
		{
			Iid = id[i];
			name = nameList[i];
			MaxQuant = quantity[i];
			...
			const char* b = image[i].c_str();
			iTextureID = CImageLoader::GetInstance()->LoadTextureGetID(b, true);
			if (iTextureID == 0)
			{
 ...
 */
void PickUP::Update(const double dElapsedTime)
{
...
*/
void PickUP::Seti32vec2Index(const int iIndex_XAxis, const int iIndex_YAxis)
{
...
*/
void PickUP::Seti32vec2NumMicroSteps(const int iNumMicroSteps_XAxis, const int iNumMicroSteps_YAxis)
{
 ...
 */
void PickUP::SetPlayer2D(CPlayer2D* cPlayer2D)
{
 ...
 */
void PickUP::Constraint(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
		if (vec2Index.x < 0)
	...
	{
		//cout << "PickUP::Constraint: Unknown direction." << endl;
	}
}
...
/**
 ...
 @param eDirection A DIRECTION enumerated data type which indicates the direction to check
 */
bool PickUP::CheckPosition(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
	...
	{
		//cout << "PickUP::CheckPosition: Unknown direction." << endl;
	}
...
// Update Jump or Fall
void PickUP::UpdateJumpFall(const double dElapsedTime)
{
...
bool PickUP::InteractWithPlayer(void)
{
	...
	// Check if the enemy2D is within 6.5 indices of the player2D
		...
		if (((vec2Index.x >= i32vec2PlayerPos.x - 1.5) &&
				...
				(vec2Index.y <= i32vec2PlayerPos.y + 1.5)))
		{
			...
			{
				if (PHK[i] == name && PHKQ[i] < MaxQuant)
				{
					cPlayer2D->addToinventory((Iid - 1000), name, 1, MaxQuant);
					bIsActive = false;
					return true;
				}
			}
 ...
 */
void PickUP::UpdateDirection(void)
{
 ...
 */
void PickUP::FlipHorizontalDirection(void)
{
	i32vec2Direction.x *= -1;
}
...
*/
void PickUP::UpdatePosition(void)
{
...
void PickUP::ShortCutPath(glm::vec2 des)
{
