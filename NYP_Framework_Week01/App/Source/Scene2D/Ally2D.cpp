...
bool Ally2D::Init(void)
{
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/dog.tga", true);
	if (iTextureID == 0)
	{
	...
	SetEscape();
	SetPotrolSpot(vec2Index);

	
	if (cMap2D->FindValue(9, uiRow, uiCol) == true)
	{
		cMap2D->SetMapInfo(uiRow, uiCol, 10);
		bowl = glm::i32vec2(uiCol, uiRow);
	}
	return true;
}
 ...
 */
void Ally2D::Update(const double dElapsedTime)
{
	if (!bIsActive)
		return;
	...
	case HUNGRY:
		...
		{
			if (!CheckPositionOfObject(bowl))
			{
				pathingShortCut(bowl);
				UpdatePosition();
			}
			else
			{//gg have fun 
	...
	case ATTACK:
		{
			...
			if (target != NULL)
			{
				pathingShortCut(target->vec2Index);

				UpdatePosition();
				InteractWithEnemy();
			}
			if (cPhysics2D.CalculateDistance(partolSpot, target->vec2Index) > 5.f)
			{
				target = NULL;
				sCurrentFSM = FINDENEMY;
				iFSMCounter = 0;
			}
			else if (target->bIsActive == false)
			{
				target = NULL;		
				sCurrentFSM = FINDENEMY;
				iFSMCounter = 0;
	...
	case FINDENEMY:
		...
		else
		{
			...
			if (!CheckPositionOfObject(partolSpot))
			{
				
				pathingShortCut(partolSpot);
				UpdatePosition();
			}
			if (cPlayer2D->GetCall(id)==1)
			{
				partolSpot = cPlayer2D->vec2Index;
			}
			if (hungry <= 0)
			{
				sCurrentFSM = HUNGRY;
...
*/
void Ally2D::Seti32vec2Index(const int iIndex_XAxis, const int iIndex_YAxis)
{
...
*/
void Ally2D::Seti32vec2NumMicroSteps(const int iNumMicroSteps_XAxis, const int iNumMicroSteps_YAxis)
{
 ...
 */
void Ally2D::SetPlayer2D(CPlayer2D* cPlayer2D)
{
 ...
 */
void Ally2D::Constraint(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
		if (vec2Index.x < 0)
	...
	{
		//cout << "Ally2D::Constraint: Unknown direction." << endl;
	}
}
...
/**
 ...
 @param eDirection A DIRECTION enumerated data type which indicates the direction to check
 */
bool Ally2D::CheckPosition(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
	...
	{
		cout << "Ally2D::CheckPosition: Unknown direction." << endl;
	}
...
// Update Jump or Fall
void Ally2D::UpdateJumpFall(const double dElapsedTime)
{
...
bool Ally2D::InteractWithEnemy(void)
{
	if (target != NULL)
	{

		glm::i32vec2 i32vec2PlayerPos = target->vec2Index;
	...
	if (sCurrentFSM==FINDENEMY)
	{
		if (((vec2Index.x >= partolSpot.x - 0.5) &&
			(vec2Index.x <= partolSpot.x + 0.5)) &&
			((vec2Index.y >= partolSpot.y - 0.5) &&
				(vec2Index.y <= partolSpot.y + 0.5)))
		{
			GetTarget(enemylist);
		}
	...
	glm::i32vec2  i32vec2PlayerPos = bowl;
	if (cMap2D->GetMapInfo(bowl.y, bowl.x) == 11)
	{

		if (((vec2Index.x >= bowl.x - 0.5) &&
			(vec2Index.x <= bowl.x + 0.5)) &&
			((vec2Index.y >= bowl.y - 0.5) &&
				(vec2Index.y <= bowl.y + 0.5)))
		{
			cout << "yetryujlkytrytykudlfukyt" << endl;
			hungry = 100;
			cMap2D->SetMapInfo(bowl.y, bowl.x, 10);
			return true;
		}
	}
	 return false;
}
void Ally2D::ESCAPE(void)
{
 ...
 */
void Ally2D::UpdateDirection(void)
{
 ...
 */
void Ally2D::FlipHorizontalDirection(void)
{
...
*/
void Ally2D::UpdatePosition(void)
{
...
void Ally2D::GetTarget(std::vector<CEnemy2D*> enemySet)
{
	for (size_t i = 0; i < enemySet.size(); i++)
	{
		if (cPhysics2D.CalculateDistance(partolSpot, enemySet[i]->vec2Index) < 5.0f && enemySet[i]->bIsActive)
		{
			target = enemySet[i];
		}
	}
...
bool Ally2D::CheckPositionOfObject(glm::vec2 something)
{
	if (((vec2Index.x >= something.x - 0.5) &&
		(vec2Index.x <= something.x + 0.5)) &&
		((vec2Index.y >= something.y - 0.5) &&
			(vec2Index.y <= something.y + 0.5)))
	{
		return true;
	}
...
void Ally2D::pathingShortCut(glm::vec2 des)
{
...
void Ally2D::SetEL(std::vector<CEnemy2D*> EL)
{
	enemylist = EL;
}

void Ally2D::SetPotrolSpot(glm::vec2 spot)
{
	partolSpot = spot;
}

void Ally2D::SetID(int i)
{
	id = i;
}

void Ally2D::AddEnemy(CEnemy2D* e)
{
